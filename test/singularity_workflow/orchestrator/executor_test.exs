if System.get_env("SINGULARITY_WORKFLOW_SKIP_DB") != "1" do
  defmodule Singularity.Workflow.Orchestrator.ExecutorTest do
    ...
    alias Singularity.Workflow.Orchestrator.Executor
    ...
    describe "execute_workflow/4" do
      test "executes workflow successfully with monitoring" do
        ...
        Mox.stub(Singularity.Workflow.Orchestrator.Repository.Mock, :create_execution, fn _, _ ->
          {:ok, %{id: "exec_123", execution_id: "exec_123", status: "running"}}
        end)

        Mox.stub(Singularity.Workflow.Executor.Mock, :execute, fn _, _, _ ->
    ...
    describe "get_execution_status/2" do
      test "returns execution status and progress" do
        ...
        Mox.stub(Singularity.Workflow.Orchestrator.Repository.Mock, :get_execution, fn _, _ ->
          {:ok, execution}
        end)

        Mox.stub(Singularity.Workflow.Orchestrator.Executor.Mock, :get_task_executions, fn _, _ ->
          {:ok,
           [
        ...
        {:ok, status} = Executor.get_execution_status("exec_123", Singularity.Workflow.Repo)
    ...
    describe "cancel_execution/3" do
      ...
      test "handles execution not found" do
        ...
        {:error, :not_found} = Executor.cancel_execution("nonexistent", Singularity.Workflow.Repo)
      end
      ...
      test "force cancels non-running execution" do
        ...
        :ok = Executor.cancel_execution("exec_123", Singularity.Workflow.Repo, force: true)
      end
    end
  end
end
