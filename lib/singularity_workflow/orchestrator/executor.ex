defmodule Singularity.Workflow.Orchestrator.Executor do
  @moduledoc """
  HTDAG-specific workflow executor with enhanced monitoring and error handling.

  Extends the base singularity_workflow executor with HTDAG-specific features:
  ...
  require Logger
  alias Singularity.Workflow.Orchestrator.Repository
  ...
  def cancel_execution(execution_id, repo, opts \\ []) do
    ...
    with {:ok, execution} <- Repository.get_execution(execution_id, repo),
      ...
      Singularity.Workflow.OrchestratorNotifications.broadcast_workflow(
        execution_id,
        :cancelled,
        %{reason: reason},
        repo
  ...
  defp execute_with_monitoring(workflow, context, execution, repo, opts) do
    ...
    case Task.await(
           Task.async(fn -> Singularity.Workflow.Executor.execute(workflow, context, repo) end),
           timeout
         ) do
      {:ok, result} ->
