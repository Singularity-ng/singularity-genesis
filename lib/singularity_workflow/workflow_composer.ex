defmodule Singularity.Workflow.WorkflowComposer do
  @moduledoc """
  Compose workflows from HTDAG decomposition.

  Provides high-level API for creating and executing goal-driven workflows
  using HTDAG task decomposition and singularity_workflow execution.
  ...
  3. **Workflow Generation**: Creates optimized singularity_workflow workflows
  ...
  - **Wraps**: Singularity.Workflow.Orchestrator, Singularity.Workflow.OrchestratorOptimizer
  ...
  User Goal Input
      ...
      │  │  │ (Orchestrator → singularity_workflow format)
      │  │  └─ Workflow + Steps
      │  │
  ...
  defp decompose_goal(goal, decomposer, opts) do
    Singularity.Workflow.Orchestrator.decompose_goal(goal, decomposer, opts)
  end

  defp create_workflow(task_graph, step_functions, opts) do
    Singularity.Workflow.Orchestrator.create_workflow(task_graph, step_functions, opts)
  end
  ...
  defp maybe_optimize_workflow(workflow, true, repo) do
    ...
    case Singularity.Workflow.OrchestratorOptimizer.optimize_workflow(
           workflow,
           repo,
